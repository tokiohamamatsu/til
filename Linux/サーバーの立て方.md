...
#### このままだとstorage内の所有者がapacheになってしまい、ユーザごとに管理したいためユーザーディレクトリでFCGIとsuexecを使う
...
そこで、ユーザーディレクトリのドキュメントルートを`/var/www/users/public_html`に変えればいいのだが、変えると`cannot get docroot infomation`とerrorが出る。
